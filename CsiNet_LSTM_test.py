...
encoded_dims = [512,512,512]  #compress rate=1/4->dim.=512, compress rate=1/16->dim.=128, compress rate=1/32->dim.=64, compress rate=1/64->dim.=32
...
for i in range(len(encoded_dims)):
    ...
    date = dates[i]
    CR2 = CR2s[i]
    file = 'CsiNet_LSTM_'+(envir)+'_dim'+str(encoded_dim)+'_'+date
    ...
    # CsiNet_LSTM_model = model_from_json(loaded_model_json)
    print('-> Build model for CR2={}...'.format(CR2))
    CsiNet_LSTM_model = CsiNet_LSTM(img_channels, img_height, img_width, T, encoded_dim, CR2, data_format=data_format)
