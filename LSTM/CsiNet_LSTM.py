...
def CsiNet_LSTM(img_channels, img_height, img_width, T, M_1, M_2, LSTM_depth=3,data_format='channels_first',t1_trainable=False,t2_trainable=True,pre_t1_bool=True,pre_t2_bool=True,aux_bool=True, lstm_latent_bool=False, pre_lstm_bool=True, conv_lstm_bool=False):
        # base CSINet models
        aux=Input((M_1,))
        ...
        config_hi = 'config/outdoor900/v1/csinet_cr512.json'
        dim, date, model_dir = unpack_compact_json(config_hi)
        network_name = get_network_name(config_hi)
        CsiNet_hi = model_with_weights(network_name,model_dir,weights_bool=pre_t1_bool,encoded_bool=True) # option to load weights; try random initialization for the network
        ...
        config_lo = 'config/outdoor900/v1/csinet_cr{}.json'.format(M_2)
        dim, date, model_dir = unpack_compact_json(config_lo)
        network_name = get_network_name(config_lo)
        CsiNet_lo = model_with_weights(network_name,model_dir,weights_bool=pre_t2_bool) # option to load weights; try random initialization for the network
        CsiNet_lo.trainable = t2_trainable
        CsiNet_lo._name = "CsiNet_lo"
        ...
        lstm_config = 'config/outdoor900/lstm_depth3_opt.json'
        dim, date, model_dir = unpack_compact_json(lstm_config)
        network_name = get_network_name(lstm_config)
        LSTM_model = model_with_weights(network_name,model_dir,weights_bool=pre_lstm_bool) # option to load weights; try random initialization for the network
        # if conv_lstm_bool:
        ...
        for i in range(T):
                ...
                if i == 0:
                        ...
                        if aux_bool:
                            ...
                            CsiNet_lo = model_with_weights(file,envir,dim,date,model_dir,weights_bool=pre_t2_bool) # option to load weights; try random initialization for the network
                        else:
                            date = "10_30"
...
def stacked_convLSTM(img_channels, img_height, img_width, T, lstm_latent_bool, LSTM_depth=3, plot_bool=False, data_format="channels_first"):
        ...
        recurrent_out = x 
        for i in range(LSTM_depth):
                recurrent_out = residual_block_LSTM(recurrent_out, orig_shape, data_format, return_sequences=True, stateful=False, recurrent_activation='sigmoid')
                print("Dim of LSTM #{} - {}".format(i+1,recurrent_out.shape))
...
def residual_block_LSTM(y,orig_shape,data_format,return_sequences=True,stateful=False,recurrent_activation='sigmoid'):
        shortcut = y
        ...
        y = ConvLSTM2D(filters=2, input_shape=orig_shape, kernel_size=(3, 3), padding='same', return_sequences=return_sequences,stateful=stateful,recurrent_activation=recurrent_activation,data_format=data_format)(y)
        y = add_common_layers(y)
        print('After y.shape: {}'.format(y.shape))

        y = ConvLSTM2D(filters=8, input_shape=orig_shape, kernel_size=(3, 3), padding='same', return_sequences=return_sequences,stateful=stateful,recurrent_activation=recurrent_activation,data_format=data_format)(y)
        y = add_common_layers(y)
        
        y = ConvLSTM2D(filters=16, input_shape=orig_shape, kernel_size=(3, 3), padding='same', return_sequences=return_sequences,stateful=stateful,recurrent_activation=recurrent_activation,data_format=data_format)(y)
        y = add_common_layers(y)
